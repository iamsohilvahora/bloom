...
class CSCO_Mega_Menu {
	...
	public function __construct() {
		...
		// Set locations.
		$this->locations = apply_filters( 'csco_mega_menu_locations', array( 'primary-menu' ) );
		...
		// Custom Fields.
		self::$fields = array(
			'csco-mega-menu' => array(
				'type'  => 'checkbox',
				'label' => esc_attr( 'Mega Menu', 'authentic' ),
			),
		);
	...
	public function mega_menu_classes( $classes, $item, $args, $depth ) {
		...
		if ( $menu_type ) {
			...
			if ( 0 === $depth ) {
				$classes[] = 'csco-mega-menu';
			}

			$classes[] = 'csco-mega-menu-' . $layout;
			...
			case 'mixed':
				if ( isset( $item->mega_menu_children ) && ! empty( $item->mega_menu_children ) ) {
					...
					if ( 'terms' === $layout ) {
								...
								<div class="cs-mm-posts-container">
									...
									foreach ( $item->mega_menu_children as $_item ) {
										if ( in_array( $_item->object, $this->taxonomies, true ) ) {
											?>
												<div class="cs-mm-posts" data-term="<?php echo esc_attr( $_item->object_id ); ?>"></div>
											<?php
										}
									}
									?>
						...
						?>
						<div class="sub-menu">
							<div class="cs-mm-posts mega-menu-posts"></div>
						</div>
						<?php
					}
				}
	...
	public function is_mega_menu( $item, $args = null ) {
		...
		// Is Item Mega Menu?
		$is_mega_menu = get_post_meta( $item->ID, 'menu-item-csco-mega-menu', true );

		if ( in_array( 'csco-mega-menu', $classes, true ) ) {
			$is_mega_menu = true;
		}
	...
	public function rest_api_callback( $request ) {
		...
		// Number of posts.
		$per_page = isset( $_GET['per_page'] ) ? (int) $_GET['per_page'] : 5;  // Input var ok; sanitization ok.
		...
		if ( $query->have_posts() && isset( $query->posts ) ) {
			...
			// Thumbnail Size Item.
			$thumbnail = apply_filters( 'csco_mega_menu_thumbnail_size', 'csco-320-landscape' );
	...
	public function admin_enqueue_scripts() {
		...
		if ( 'nav-menus.php' === $pagenow ) {
			...
			<style type="text/css">
				...
				#menu-to-edit li.menu-item:not( .menu-item-depth-0 ) .field-csco-mega-menu {
					display: none;
				}
			</style>
			<?php
