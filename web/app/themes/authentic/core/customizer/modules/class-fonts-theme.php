...
if ( ! class_exists( 'CSCO_Customizer_Fonts_Theme' ) ) {
	...
	final class CSCO_Customizer_Fonts_Theme {
			...
			if ( is_customize_preview() ) {
				...
				if ( $this->fonts_output ) {
					$fonts['fonts']['families']['theme'] = array(
						'text'     => esc_html__( 'Theme Fonts', 'authentic' ),
						'children' => array(),
					);
				}
		...
		public function get_styles_theme_fonts( $method = null ) {
			...
			foreach ( $this->fonts_output as $slug => $font ) {
				...
				foreach ( $font['variants'] as $variant ) {
					...
					// Get font path.
					$font_path = sprintf( '%s/assets/static/fonts/%s-%s', get_theme_file_uri(), $slug, $variant );
		...
		public function enqueue_theme_fonts( $page ) {
			...
			wp_register_style( 'csco-theme-fonts', false, array(), csco_get_theme_data( 'Version' ) );

			wp_enqueue_style( 'csco-theme-fonts' );

			wp_add_inline_style( 'csco-theme-fonts', $this->get_styles_theme_fonts() );
		}
	}
